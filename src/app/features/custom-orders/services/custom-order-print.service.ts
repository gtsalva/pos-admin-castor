...
export class CustomOrderPrintService {
  ...
  generate(order: CustomOrder, printedAt: Date): Blob {
    ...
    const m  = 14;
    const rEdge   = pW - m;     // right margin
    const colQty  = rEdge - 56; // qty right edge
    const colUnit = rEdge - 28; // unit price right edge
    let y = 14;
    ...
    const divider = (color = 200): void => {
      doc.setDrawColor(color); doc.line(m, y, rEdge, y); y += 5;
    };
    const labelVal = (label: string, val: string, fontSize = 9): void => {
    ...
    doc.text(`${percent}% pagado${order.agreed_price != null ? ' (sobre total acordado)' : ''}`, rEdge, y, { align: 'right' });
    y += 7;
    ...
    if (order.payments.length > 0) {
      ...
      for (const p of order.payments) {
        need(7);
        const d = new Date(p.created_at).toLocaleDateString('es-GT');
        doc.text(d,                                m,         y);
