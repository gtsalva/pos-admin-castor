...
interface ReceiveRow extends PurchaseOrderItem {
...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, NzTableModule, NzButtonModule, NzInputNumberModule, NzAlertModule, NzSpinModule, FormsModule, CurrencyPipe],
  template: `
          ...
          <tbody>
            @for (row of rows(); track row.purchase_item_id; let i = $index) {
              <tr>
                ...
                <td>
                  <nz-input-number
                    [ngModel]="row.qty_received_input"
                    (ngModelChange)="updateRowQty(i, $event)"
                    [nzMin]="0"
                ...
                <td>{{ row.qty_received_input * row.unit_cost | currency:'GTQ':'symbol':'1.2-2':'es-GT' }}</td>
              </tr>
            }
          </tbody>
        </nz-table>
...
export class PurchasesReceiveComponent implements OnInit {
  ...
  readonly rows = signal<ReceiveRow[]>([]);
    ...
    this.api.getOne(this.purchaseId()).subscribe({
      next: (po) => {
        ...
        this.rows.set(
          po.items.map((item) => ({
            ...item,
            qty_received_input: item.quantity_ordered,
          })),
        );
  ...
  updateRowQty(index: number, value: number): void {
    this.rows.update((list) =>
      list.map((row, i) => (i === index ? { ...row, qty_received_input: value } : row)),
    );
  }
  ...
  confirm(): void {
    ...
    const payload = {
      items: this.rows().map((r) => ({
        ...
        quantity_received: r.qty_received_input,
      })),
    };
